...
const en = {
  ...
  settings: {
    ...
    simMaxAge: 'Max sim age (hours, min 4; older → S = 0)',
    ...
    casualMultiplier: 'Pepega multiplier',
    ...
    formulaNote: (casual: string | null) =>
      `divided by (1 + L), times A and the effort factor F; Pepega A = ${casual},`,
  ...
  standings: {
    ...
    casualCount: 'Marked Pepega',
    recentLoot: 'Items in loot window',
    staleWishlists: (hours: number) =>
      `Stale droptimizers (>${hours % 24 === 0 ? `${hours / 24}d` : `${hours}h`})`,
  ...
  history: {
    ...
    otherBoss: 'Other / non-raid',
    searchPlaceholder: 'Search item or player…',
  ...
  rules: {
    ...
    activityDescRegular: '— Blaster: pulls their weight — shows up, performs, unlocks Vault slots.',
    activityDescCasual: (penalty: string | null) =>
      `— Pepega: raid-logger, trials, frequent absences, or underperforming (${penalty}% penalty). Set by the loot council.`,
    ...
    exampleSub:
      'An item drops. Neither player has recent loot (L = 0) and both are "Blaster" (A = 1.0).',
...
const bg: Strings = {
  ...
  settings: {
    ...
    simMaxAge: 'Макс. възраст на sim (часове, мин 4; по-стар → S = 0)',
    ...
    casualMultiplier: 'Множител за Пепега',
    ...
    mythic: 'Mythic',
    formulaNote: (casual: string | null) =>
      `делено на (1 + L), по A и ефорт фактора F; Пепега A = ${casual},`,
  ...
  standings: {
    ...
    staleWishlists: (hours: number) =>
      `Остарели droptimizer-и (>${hours % 24 === 0 ? `${hours / 24}д` : `${hours}ч`})`,
    search: 'Търси играч…',
    colPlayer: 'Играч',
    colRole: 'Роля',
    colIlvl: 'ilvl',
  ...
  history: {
    ...
    allBosses: 'Всички босове',
    allDifficulties: 'Всички трудности',
    allResponses: 'Всички отговори',
    otherBoss: 'Друго / извън рейда',
    searchPlaceholder: 'Търси предмет или играч…',
  ...
  rules: {
    ...
    activityDescRegular: '— Бластър: дърпа тежестта си — идва на рейд, пърформва и отключва слотове във Vault.',
    activityDescCasual: (penalty: string | null) =>
      `— Пепега: raid-logger, нови trial-и, чести отсъствия или ъндърпърформване (${penalty}% наказание). Определя се от loot council-а.`,
    ...
    exampleSub:
      'Пада предмет. И двамата играчи нямат скорошен луут (L = 0) и са със статус „Бластър“ (A = 1.0).',
